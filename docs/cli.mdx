...
### Aligning GeoTIFFs to a reference

`ddb align` corrects the georeferencing offset of an orthophoto or DEM by aligning it to a more accurate reference raster.

```bash
ddb align -i source.tif -r reference.tif -o aligned.tif
```

The output is always a Cloud Optimized GeoTIFF reprojected to EPSG:3857.

**Alignment modes:**

| Mode | Description |
|------|-------------|
| `similarity` (default) | 4-DOF transform: translation X/Y + rotation + uniform scale. Best for georeferencing corrections. |
| `translation` | 2-DOF transform: translation X/Y only. Faster, use when no rotation or scale error is expected. |

```bash
# Fast translation-only alignment
ddb align -i source.tif -r reference.tif -o aligned.tif -m translation

# Validate compatibility without performing the alignment
ddb align -i source.tif -r reference.tif --validate
```

The `--validate` flag checks that the source and reference are compatible (same CRS class, sufficient spatial overlap, compatible raster type) and prints a JSON result:
...
```json
{ "ok": false, "errors": ["Insufficient overlap between source and reference"] }
```

The `--no-seed` flag disables the phase-correlation seed step. This makes the initial NCC search slower but can improve robustness on low-contrast rasters.
